...
router.get('/tribe-markers/:regionId', auth, uuidParam('regionId'), async (req, res) => {
  ...
  try {
    const rows = await prisma.tribeMarker.findMany({
      where: config.sharedTribeMarkers ? { regionId } : { regionId, placedBy: req.user.id },
    ...
    res.json({
      markers: rows.map(marker => ({
        ...
        region_id: regionId,
        is_own: marker.placedBy === req.user.id,
...
router.post('/tribe-markers', auth, body(postTribeMarkerSchema), async (req, res) => {
  ...
  try {
    ...
    if (!tribeTypeId) return res.status(400).json({ error: 'Invalid type' })
    ...
    if (err.code === 'P2034') return res.status(409).json({ error: 'Write conflict, please retry' })
    console.error(err)
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
router.patch('/tribe-markers/:id', auth, uuidParam('id'), body(patchTribeMarkerSchema), async (req, res) => {
  ...
  try {
    ...
    if (type !== undefined && !tribeTypeId) return res.status(400).json({ error: 'Invalid type' })
