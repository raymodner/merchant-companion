...
router.get('/tribe-types', async (_req, res) => {
  try {
    const tribeTypes = await prisma.tribeType.findMany({ orderBy: { sortOrder: 'asc' }, select: { name: true } })
    res.json({ tribeTypes: tribeTypes.map(t => t.name) })
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: 'Internal server error' })
  }
