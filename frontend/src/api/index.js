...
export const api = {
  ...
  async loadTerrain(regionId) {
    try {
      const res = await fetch(`${BASE}/terrain/${regionId}`, { credentials: 'include' })
      if (!res.ok) return {}
  ...
  async paintCell(regionId, cellKey, terrainKey) {
    try {
      const res = await fetch(`${BASE}/terrain/${regionId}/cell`, {
        method: 'POST',
        headers: json,
        credentials: 'include',
  ...
  async getResources() {
    try {
      const res = await fetch(`${BASE}/resources`, { credentials: 'include' })
  ...
  async getRegions() {
    try {
      ...
      return res.ok ? res.json() : { countries: {}, subregions: {} }
    } catch { return { countries: {}, subregions: {} } }
  },
  ...
  async getTribeTypes() {
    try {
      const res = await fetch(`${BASE}/tribe-types`)
      return res.ok ? res.json() : { tribeTypes: [] }
    } catch { return { tribeTypes: [] } }
  },
