          ...
          >
            ...
            >
              ...
              >
                ...
                <div class="res-loc-stars-wrap">
                  <div v-if="!rs.editMode || !authStore.user" class="res-loc-stars-toggle" @click="setStar(loc.id, loc.stars === 0 ? 1 : 0)">
                  ...
                  <template v-else>
                    <button class="star-btn star-btn-reset" :class="{ active: loc.stars === 0 }" @click="setStar(loc.id, 0)">✕</button>
                    <button
                      v-for="n in [1,2,3,4,5]"
                      :key="n"
                      :class="['star-btn', n <= loc.stars ? 'filled' : '']"
                      @click="setStar(loc.id, n)"
                    >{{ n <= loc.stars ? '★' : '☆' }}</button>
                  </template>
                </div>
              </div>
